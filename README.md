...
| TypeDB KG RAG | 3건 부분 (1급EX, 2급EX, 1급EE) | ⚠️ | TypeQL 실행은 6건이나 LLM 답변 생성 시 일부 누락 |
...
- **TypeDB 범위 필터 답변 누락**: Q3에서 TypeQL 규칙 기반 플래너가 6건을 정확히 조회하지만, LLM 답변 생성 단계에서 일부 행이 누락되는 현상이 있음. 답변 생성 프롬프트 최적화 또는 구조화된 결과 직접 반환 방식 검토 필요.
