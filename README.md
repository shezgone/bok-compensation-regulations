...
- Neo4j `nl_query.py`에 **Hybrid GraphRAG Router** 적용 (`Data` / `Semantic` 분기)
- TypeDB `nl_query.py`에도 동일한 **시멘틱 라우터** 적용
- `조문`/`:조문` 컨텍스트 기반 규정 해석 답변 경로 추가
- 라우터 단위 테스트 추가: [tests/test_nl_router.py](tests/test_nl_router.py), [tests/test_typedb_router.py](tests/test_typedb_router.py)
- 실동작 확인 질문: "기한부 고용계약자는 상여금을 받을 수 있어?" → TypeDB/Neo4j 모두 시멘틱 경로로 정상 응답
...
│   ├── test_nl_router.py             # Neo4j Hybrid Router 분기 단위 테스트
│   └── test_typedb_router.py         # TypeDB Hybrid Router 분기 단위 테스트
├── pyproject.toml
└── README.md
```
...
# Neo4j 파이프라인까지 실행하려면 Python Neo4j 드라이버도 설치 필요
pip install neo4j
```
...
# TypeDB Hybrid Router - 규정 해석형 질문
PYTHONPATH=src python -m bok_compensation.nl_query \
...
#### Hybrid GraphRAG Router

TypeDB와 Neo4j 파이프라인 모두 **의도 분류 기반 Hybrid Router**를 사용하며, 질문 유형에 따라 서로 다른 경로로 처리합니다.

```text
질문
 ├─ Data intent     → TypeQL/Cypher 생성 → DB 조회 → 수치 답변 생성
 └─ Semantic intent → 조문 서브그래프 조회 → 규정 컨텍스트 기반 답변 생성
```

- `Data` 질문: 본봉, 직책급, 지급률, 차등액, 상한액, 비교/계산 등 **수치 중심 질의**
- `Semantic` 질문: 지급 가능 여부, 적용 대상, 규정 해석, 조건/예외 등 **규정 해석 질의**
- TypeDB Semantic 경로에서는 `조문` 엔티티의 `조번호`, `조문내용`을 컨텍스트로 수집합니다.
- Neo4j Semantic 경로에서는 `:조문` 노드의 `조번호`, `조문내용`을 컨텍스트로 수집합니다.
- 기존 TypeQL/Cypher 생성 경로는 유지되어, 수치형 질문에는 기존 정확도를 그대로 활용합니다.
...
- **Hybrid Router** (`classify_intent`, `get_rules_subgraph`, `semantic_answer`): TypeDB/Neo4j 모두에서 정량 질의와 규정 해석 질의를 분기 처리
...
| G5 직급의 초봉은? | ✅ | ✅ |
| 일반사무직원의 초봉은? | △ (LLM 생성 편차 가능) | ✅ |
| 기한부 고용계약자는 상여금을 받을 수 있어? | ✅ (Semantic Router) | ✅ (Semantic Router) |
...
# Neo4j Hybrid Router 분기 단위 테스트
PYTHONPATH=src pytest tests/test_nl_router.py

# TypeDB Hybrid Router 분기 단위 테스트
PYTHONPATH=src pytest tests/test_typedb_router.py
```
...
Hybrid Router 단위 테스트: Semantic 경로와 Data 경로가 각각 올바른 함수 체인으로 분기되는지 검증

최근 검증 결과:

- `pytest tests/test_nl_router.py tests/test_typedb_router.py` → 4건 통과
- TypeDB `src/bok_compensation/nl_query.py` 직접 실행으로 Semantic 질의 응답 확인
- Neo4j `src/bok_compensation_neo4j/nl_query.py` 직접 실행으로 Data/Semantic 질의 응답 확인
...
- **Neo4j 권장 이유**
  - Cypher가 산술/집계를 직접 지원해 정량 질의 처리에 강함
  - LLM이 Cypher를 더 안정적으로 생성하는 경향이 있음
  - 현재 검증 기준으로 `일반사무직원의 초봉은?` 같은 질문에서 Neo4j 쪽 응답 안정성이 더 높음
...
- **Python 3.9+** — 런타임## 🧪 TypeDB vs Neo4j 질의 처리 능력 평가 (10개 복합 질의 테스트)

보수 규정과 관련된 복합적인 연산 및 조건이 포함된 10개의 테스트 질의를 통해 TypeDB와 Neo4j 기반 자연어 질의 파이프라인의 성능을 각각 평가했습니다.

### 1️⃣ 테스트 결과 요약 (정답률: 양쪽 모두 30%)

| 연번 | 질문 요약 (기대 정답) | TypeDB 결과 | Neo4j 결과 |
| :--- | :--- | :--- | :--- |
| 1 | G3 기본 연봉 상한액 (77,724,000) | **✅ 성공** | **✅ 성공** |
| 2 | G1 EX 상여금 지급률 (150%) | ❌ 오답 (100% 도출) | ❌ 오답 (빈 값) |
| 3 | 1급 보직자 직책급 월액 (420,000) | ❌ 오답 (연 총액 도출) | ❌ 오답 (연 총액 도출) |

## 🧪 TypeDB vs Neo4j 질의 처리 능력 평가 (10개 복합 질의 테스트)
...
| 2 | G1 EX 상여금 지급률 (150%) | ❌ 오답 (100% 도출) | ❌ 오답 (빈 값) |
| 3 | 1급 보직자 직책급 월액 (420,000) | ❌ 오답 (연 총액 도출) | ❌ 오답 (연 총액 도출) |
| 4 | C1~C3 NI 등급 상여금 (0%) | ❌ 에러 (OR 구문 파싱 오류) | ❌ 오답 (빈 값) |
| 5 | G5 초임 호봉 (11호봉) | **✅ 성공** | **✅ 성공** |
| 6 | G2 직급 연봉 상한액 (78,540,000) | **✅ 성공** | **✅ 성공** |
| 7 | C1 상여금 기준 금액 (1,864,000) | ❌ 에러 (산술 연산 에러) | ❌ 오답 (빈 값) |
| 8 | 등급 배열 배분 비율 (15, 10, 25) | ❌ 오답 ([0, 0, 0] 도출) | ❌ 오답 (빈 값) |
| 9 | G4, G5 시간외근무 (209시간) | ❌ 에러 (타입 매칭 에러) | ❌ 에러 (JSON 특수문자 파싱) |
| 10 | 1급 연봉제 하한액 (54,012,000) | ❌ 에러 (덧셈 구문 에러) | ❌ 오답 (빈 값) |

### 2️⃣ 실패 원인 및 모델의 한계 비교

비록 양쪽 모두 맞힌 문항의 개수는 3개로 동일하지만, **틀리는 '실패 패턴'이 완전히 다릅니다.**

*   **TypeDB의 실패 패턴 (엄격한 Syntax & Type 검증)**
    *   문법(Syntax)과 검증이 매우 까다로워 `+`, `*` 같은 수학 연산이나 `OR` 논리 연산을 시도할 때 **실행 단계 이전 체커에서 파싱 에러(Syntax Error, Type Inference Error)**가 발생하며 중단됩니다.
    *   LLM이 Schema 구조를 완전히 이해하지 못한 상태에서는 실행 가능한 쿼리를 한 번에 도출해 내기가 극도로 어렵습니다.
*   **Neo4j의 실패 패턴 (유연한 Cypher 실행)**
    *   Cypher 언어는 유연하여 LLM이 날리는 쿼리 대부분이 파싱 에러 없이 "실행"은 통과됩니다. 
    *   하지만 정교한 그래프 스키마 체계 속에서 관계선(`[:해당직급]->` 등)이나 노드의 세부 프로퍼티를 한 군데라도 잘못 짚으면, **에러 없이 그냥 "조회 결과 없음(빈 값)"을 돌려버리는 사일런트 실패(Silent Failure)**가 주로 발생합니다.

### 3️⃣ 근본적 병목 지점

현재 파이프라인에 적용된 **로컬 소형 파라미터 LLM(`qwen2.5-coder:14b`)의 추론 한계**가 가장 큰 병목입니다. 
한국은행 보수규정처럼 "호봉체계구성 -> 연봉제상한 -> 지급률 -> 보수유형" 식으로 복잡하게 중첩된 Multi-hop 형태의 그래프 구조를 LLM이 단일 Prompt와 Schema 메타데이터만 훑어보고 완벽한 쿼리(Cypher 또는 TypeQL)로 한 번에 컴파일 해내는 데는 명백한 한계가 존재합니다.

### 4️⃣ 향후 개선 팁 (Next Steps)

1.  **Few-shot 예시 프롬프팅 도입:** 실무 적용 시, 단순 Zero-shot 스키마 주입 방식 대신 도메인별(예: 기본급, 상여금, 수당)로 **검증된 정답 쿼리 예시(Few-shot)를 3~4개씩 포함해 제공해 주면 정답률을 드라마틱하게 올릴 수 있습니다.**
2.  **라우터의 세분화:** 현재 `Data` vs `Semantic`으로 2분할 된 라우팅 시스템의 `Data` 의도를 한 번 더 세분화하여, 수당용 파이프라인, 상여금 파이프라인, 기본급 파이프라인 등으로 나누어 쿼리 범위를 축소시키는 것이 안정적입니다.
3.  **검증 단계 파이프라인 (Reflexion):** Neo4j가 빈 값을 반환했을 때 LLM에게 "이 Cypher 쿼리의 엣지가 잘못된 것 같으니 다른 관계로 다시 탐색해보라"고 피드백을 주는 **자가 수정 회귀(Self-correction Reflexion)** 로직 추가가 필요합니다.
