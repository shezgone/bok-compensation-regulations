...
#### Neo4j Hybrid GraphRAG Router

Neo4j 파이프라인에는 **의도 분류 기반 Hybrid Router**가 추가되어, 질문 유형에 따라 서로 다른 경로로 처리합니다.
...
- Semantic 경로에서는 `:조문` 노드의 `조번호`, `조문내용`을 컨텍스트로 수집해 환각을 줄입니다.
- 기존 Cypher 생성 경로는 유지되어, 수치형 질문에는 기존 정확도를 그대로 활용합니다.
...
- **Neo4j Hybrid Router** (`classify_intent`, `get_rules_subgraph`, `semantic_answer`): 정량 질의와 규정 해석 질의를 분기 처리
...
| G5 직급의 초봉은? | ✅ | ✅ |
| 일반사무직원의 초봉은? | ✅ | ✅ |
| 기한부 고용계약자는 상여금을 받을 수 있어? | - | ✅ (Semantic Router) |
