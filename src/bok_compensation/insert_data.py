...
def insert_regulation(driver, db):
    ...
    run_query(driver, db, """
        insert $reg isa 규정,
            ...
            has 명칭 "보수규정",
            has 설명 "한국은행법과 한국은행정관에 따라 금융통화위원회 위원, 집행간부, 감사 및 직원의 보수 및 상여금에 관한 사항을 규정",
...
def insert_bonus_standards(driver, db):
    ...
    run_query(driver, db, """
        insert $b isa 상여금기준, has 코드 "BONUS-REG",
            ...
            has 연간지급률 3.8,
            has 설명 "연간 380%. 6·12월 각 150%, 설·추석 각 40%";
    """)
    ...
    for year, rate in WAGE_PEAK_TABLE:
        ...
        run_query(driver, db, f"""
            insert $w isa 임금피크제기준, has 코드 "{code}",
                ...
                has 설명 "별표9 임금피크제 적용연차 {year}년차 기본급지급률 {int(rate*100)}%";
        """)
