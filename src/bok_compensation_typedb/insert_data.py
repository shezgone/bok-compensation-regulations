...
# insert_regulation — 제거됨 (규정 텍스트는 context RAG에서 처리)


# insert_amendment_history — 제거됨 (개정이력은 context RAG에서 처리)


# insert_articles, insert_article_relations — 제거됨 (조문 텍스트는 context RAG에서 처리)
...
def insert_addendum_overrides(driver, db):
    ...
    # ── 3) 부칙 제2조 → 조문 대체 — 제거됨 (조문 텍스트는 context RAG에서 처리) ──
